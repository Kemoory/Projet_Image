...
imgSeuil = seuilGray(imgSobel, find_threshold_kmeans(imgSobel))
if imgSeuil.dtype != np.uint8:  # Vérifier le type de données de l'image
    print("converti")
    imgSeuil = imgSeuil.astype(np.uint8)  # Convertir en CV_8UC1 si nécessaire

img_copy = img.copy()

circles=cv2.HoughCircles(imgSeuil,cv2.HOUGH_GRADIENT,1,2,param1=200,param2=60,minRadius=10,maxRadius=100)
...
if circles is not None:
    ...
    """
    cv2.imshow("Cercles détectés", img_copy)
    cv2.waitKey(0)
